...
// #include "/usr/local/include/apriltag/apriltag.h"
// #include "apriltag.h"
...
public:
   ...
   */
  explicit TrackApril(std::unordered_map<size_t, std::shared_ptr<CamBase>> cameras, int numaruco, bool stereo, HistogramMethod histmethod,
                      bool downsize)
...
//     // April dectiontion 수행하는 객체 초기화 및 detection 객체 정의 
//     // initialize_april_detector() ;
// #endif
// #else
...
// #endif
  ...
  // void initialize_april_detector();
  // void detectMarkers(cv::Mat &inputImage, std::vector<std::vector<cv::Point2f>>& corners, std::vector<int> ids_april);
  void detectMarkers(apriltag_detector_t *tagDetector_ , cv::Mat &inputImage, std::vector<std::vector<cv::Point2f>>& corners, std::vector<int>& ids_april);
...
// 
   ...
   */
  void perform_april_tracking(double timestamp, const cv::Mat &imgin, size_t cam_id, const cv::Mat &maskin);
#endif
